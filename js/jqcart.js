...
;(function($) {
  ...
  var cartData,
    ...
    orderform = '<p class="jqcart-cart-title"><form class="jqcart-orderform"><input type="reset" value="Вернуться к покупкам"></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
    ...
    openCart: function() {
      ...
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
          ...
          orderPreview += '<div class="jqcart-sum">' + sum + ' ' + 'руб' + '</div>';
					orderPreview += '<div class="jqcart-small-td"><span class="jqcart-del-item"></span></div>';
          orderPreview += '</div>';
        }
      }
      orderPreview += '</div></div>';
      orderPreview += '<div class="jqcart-subtotal">Итого: <strong>' + subtotal + '</strong> ' + 'руб'+ '</div>';
